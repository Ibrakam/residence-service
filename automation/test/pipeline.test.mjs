...
async function runRejectedAgentFixture(t, actionCode, ticketId) {
...
test("a deployment failure rolls main back with a commit lease", async (t) => {
  ...
  const config = {
    ...
    verifyCommands: [{
      name: "node-smoke",
      cwd: ".",
      argv: [process.execPath, "-e", `const fs=require("fs"),p=require("path");const r="website/dist/standalone";fs.mkdirSync(p.join(r,"dist/client"),{recursive:true});for(const [f,v] of [["server.js","ok"],["package.json","{}"],["STANDALONE_RUNTIME.json","{}"]])fs.writeFileSync(p.join(r,f),v)`],
      timeoutMs: 5_000,
    }],
    verificationEnvironment: {},
    gitAuthorName: "Ticket Test",
