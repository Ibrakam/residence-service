...
func catalogDeveloper(slug string, project map[string]json.RawMessage) (string, string) {
	if developer := firstString(project, "developerSlug"); developer != "" {
		...
		if developer == "kayan" {
			name = "KAYAN"
		} else if developer == "murad-buildings" {
	...
	case "mirador", "ofiyat":
		return "kayan", "KAYAN"
	case "c1", "regnum-plaza", "saadiyat", "sarbon", "soy-boyi":
...
func projectName(slug string) string {
	names := map[string]string{
		...
		"saadiyat": "SAADIYAT", "sarbon": "SARBON", "sado": "Sad'O", "soy-boyi": "Soy Bo‘yi", "sun": "SUN", "voha": "Voha", "yangibaxt": "Yangi Baxt",
	}
