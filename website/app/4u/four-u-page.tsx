...
const copy = {
  ...
  uz: {
    ...
    buildKicker: '07 · CONSTRUCTION', buildTitle: 'Qurilish — renderlar bilan almashtirilmagan.', buildText: '2026-yil iyul rasmiy hisoboti: uch blokda monolit ishlari davom etgan, 2- va 3-bloklarda ikkinchi qavat yopmasi betonlangan.', camera: 'Onlayn kamera', cameraStatus: 'Rasmiy saytda vaqtincha ishlamaydi: texnik ishlar olib borilmoqda.',
  ...
  en: {
    ...
    buildKicker: '07 · CONSTRUCTION', buildTitle: 'Construction, clearly separated from renders.', buildText: 'Official July 2026 report: monolithic works were underway in three blocks, with second-floor slab concreting in blocks 2 and 3.', camera: 'Live camera', cameraStatus: 'Temporarily unavailable on the official site due to maintenance.',
...
export function FourUPage({ initialLanguage = 'ru' }: { initialLanguage?: Language }) {
  ...
  return <main className="fouru-site" lang={language}>
    ...
    <section className="fouru-catalog"><div><span className="fouru-kicker">{t.catalogKicker}</span><h2>{t.catalogTitle}</h2><p>{t.catalogText}</p><div><a className="fouru-button is-coral" href={withLanguage('/4u/apartments', language)}>{t.catalogOpen}<span>↗</span></a><a className="fouru-button is-light" href={`${appBasePath}/4u/4u-tashkent-booklet.pdf`} target="_blank">{t.booklet}<span>↓</span></a></div></div><aside><span>183</span><small>OFFICIAL<br />LISTINGS</small><i /><span>33</span><small>LOCAL<br />SNAPSHOT</small></aside></section>

    <section className="fouru-contact"><div><span className="fouru-kicker is-coral">{t.contactKicker}</span><h2>{t.contactTitle}</h2><p>{t.contactText}</p></div><div className="fouru-contact__actions"><button className="fouru-button is-coral" type="button" onClick={() => setLead(true)}>{t.consult}<span>↗</span></button><a className="fouru-button is-outline" href="tel:1360">{t.phone}<span>↗</span></a></div></section>
    <footer className="fouru-footer"><a className="fouru-logo" href="#top"><b>4U</b><span>TASHKENT</span></a><p>{t.partner}</p><div><a href="https://nrg-bi.uz/uz-ru/landing/4u-tashkent" target="_blank" rel="noreferrer">{t.source}</a><a href={`${appBasePath}/privacy?project=4u&lang=${language}`}>Privacy</a></div><small>{t.legal}</small></footer>
