...
const copy = {
  ru: { title: 'C1 — премиальная башня в центре Ташкента', description: 'C1 в Яшнабадском районе: 30 этажей, 252 квартиры, бассейн, фитнес и автоматически обновляемый каталог. Сдача — I квартал 2028.', imageAlt: 'Архитектурная CGI-визуализация башни C1', home: 'Главная' },
  uz: { title: 'C1 — Toshkent markazidagi premium minora', description: 'Yashnobod tumanidagi C1: 30 qavat, 252 xonadon, basseyn, fitness va avtomatik yangilanadigan katalog. Topshirish — 2028-yil I chorak.', imageAlt: 'C1 minorasining arxitektura CGI-vizualizatsiyasi', home: 'Bosh sahifa' },
  en: { title: 'C1 — a premium tower in central Tashkent', description: 'C1 in Yashnabad District: 30 floors, 252 apartments, pool, fitness and an automatically updated catalogue. Completion — Q1 2028.', imageAlt: 'Architectural CGI visualisation of the C1 tower', home: 'Home' },
} as const;
...
export default async function Page({ searchParams }: PageProps) {
  ...
  const previewUnits = catalog.units.slice(0, 4).map((unit) => ({ id: unit.id, number: unit.number, rooms: unit.rooms, area: unit.area, floor: unit.floor, section: unit.section, phase: unit.phase, completionYear: unit.completionYear, plan: unit.plan }));
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData).replace(/</g, '\\u003c') }} /><C1Page initialLanguage={language} previewUnits={previewUnits} initialAvailableCount={catalog.availableTotal} /></>;
}
