...
const copy = {
  ru: {
    ...
    modes: { cards: 'Карточки', chess: 'Шахматка', 'chess-plus': 'Шахматка+' }, language: 'Язык', filters: 'Фильтры', phase: 'Очередь / тип', allTypes: 'Все предложения', apartmentType: 'Готовый дом · квартиры', rooms: 'Комнаты', allRooms: 'Все', areaFrom: 'Площадь от', areaTo: 'Площадь до', status: 'Статус', allPublished: 'Все опубликованные', available: 'В продаже', floor: 'Этаж', allFloors: 'Все этажи', sort: 'Сортировка', reset: 'Сбросить', result: 'предложений',
    sorts: { priceAsc: 'Сначала дешевле', priceDesc: 'Сначала дороже', areaAsc: 'Площадь по возрастанию', areaDesc: 'Площадь по убыванию', floorAsc: 'Сначала нижние этажи' }, snapshot: 'Официальный snapshot', captured: 'Срез каталога', sourceNote: 'В snapshot входят только 8 предложений, которые официальный каталог показывал на дату проверки. Снятые, проданные и забронированные квартиры не реконструируются.', official: 'Открыть официальный каталог',
  ...
  uz: {
    ...
    modes: { cards: 'Kartalar', chess: 'Shaxmatka', 'chess-plus': 'Shaxmatka+' }, language: 'Til', filters: 'Filtrlar', phase: 'Navbat / tur', allTypes: 'Barcha takliflar', apartmentType: 'Tayyor uy · xonadonlar', rooms: 'Xonalar', allRooms: 'Barchasi', areaFrom: 'Maydon, dan', areaTo: 'Maydon, gacha', status: 'Holat', allPublished: 'Barcha e’lonlar', available: 'Sotuvda', floor: 'Qavat', allFloors: 'Barcha qavatlar', sort: 'Saralash', reset: 'Tozalash', result: 'ta taklif',
    sorts: { priceAsc: 'Arzonidan boshlab', priceDesc: 'Qimmatidan boshlab', areaAsc: 'Maydon o‘sishi bo‘yicha', areaDesc: 'Maydon kamayishi bo‘yicha', floorAsc: 'Quyi qavatlar avval' }, snapshot: 'Rasmiy snapshot', captured: 'Katalog snapshoti', sourceNote: 'Snapshotga tekshiruv sanasida rasmiy katalog ko‘rsatgan 8 ta taklif kiritilgan. E’londan olingan, sotilgan va band xonadonlar qayta yaratilmaydi.', official: 'Rasmiy katalogni ochish',
...
export function FlagmanCatalogPage({ snapshot, initialLanguage = 'ru' }: { snapshot: FlagmanCatalogSnapshot; initialLanguage?: Language }) {
  ...
  return <main className="flagman-catalog" lang={language}>
    ...
    <section className="flagman-catalog-toolbar" aria-label={t.filters}>
      <div className="flagman-mode-switch" role="radiogroup" aria-label={t.modes.cards}>{(['cards', 'chess', 'chess-plus'] as Mode[]).map((item) => <button data-testid={`flagman-mode-${item}`} type="button" role="radio" aria-checked={mode === item} className={mode === item ? 'is-active' : ''} key={item} onClick={() => setMode(item)}>{t.modes[item]}</button>)}</div>
    ...
    <section className="flagman-results">
      ...
      <div className="flagman-source-note"><span>i</span><p>{t.sourceNote} <b>{t.sourceCaveat}</b></p><a href={snapshot.source.catalog} target="_blank" rel="noreferrer">{t.official} ↗</a></div>
      ...
      {mode !== 'cards' && filtered.length ? <section className="flagman-matrix"><header><div><small>{snapshot.project.class}</small><h2>{t.offers}</h2></div><div><button data-testid="flagman-scroll-left" type="button" aria-label={t.previous} onClick={() => scrollMatrix(-1)}>←</button><button data-testid="flagman-scroll-right" type="button" aria-label={t.next} onClick={() => scrollMatrix(1)}>→</button></div></header><div className={mode === 'chess-plus' ? 'flagman-matrix-plus' : ''}><div ref={scrollRef} className="flagman-matrix-scroll" tabIndex={0} aria-label={t.offers} onKeyDown={onMatrixKey}><div className="flagman-matrix-table"><div className="flagman-matrix-caption"><span>{t.floor}</span><strong>{t.offers}</strong></div>{matrixFloors.map((value) => <div className="flagman-matrix-row" key={value}><div className="flagman-matrix-floor"><strong>{value}</strong><span>{t.floor}</span></div><div className="flagman-matrix-cells">{filtered.filter((unit) => unit.floor === value).sort((a, b) => Number(a.number) - Number(b.number)).map((unit) => <button type="button" key={unit.id} className={selected?.id === unit.id ? 'is-selected' : ''} aria-pressed={selected?.id === unit.id} onClick={() => setSelectedId(unit.id)}><small>№ {unit.number}</small><strong>{mode === 'chess-plus' ? `${unit.rooms} · ${unit.area.toFixed(2)} м²` : `${unit.area.toFixed(1)} м²`}</strong>{mode === 'chess-plus' ? <span>{money(unit.price, language)}</span> : null}</button>)}</div></div>)}</div></div>{mode === 'chess-plus' && selected ? <UnitDetail unit={selected} language={language} onPlan={() => setPlanUnit(selected)} onLead={() => setLeadUnit(selected)} /> : null}</div>{mode === 'chess' && selected ? <div className="flagman-matrix-selection"><div><small>{t.selected}</small><strong>№ {selected.number} · {selected.area.toFixed(2)} м²</strong><span>{selected.floor}/{selected.maxFloor} {t.floor} · {money(selected.price, language)}</span></div><button type="button" onClick={() => setPlanUnit(selected)}>{t.plan}</button><button className="is-primary" type="button" onClick={() => setLeadUnit(selected)}>{t.choose}</button></div> : null}</section> : null}
    </section>
