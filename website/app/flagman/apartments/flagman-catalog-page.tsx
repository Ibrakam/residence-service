...
const copy = {
  ru: {
    back: 'О проекте', project: 'FLAGMAN · ГОТОВЫЙ ДОМ', title: 'Выберите квартиру', accent: 'в частной галерее.', lead: 'Реальные предложения официального каталога — с точными планировками, площадями, этажами и ценами по актуальным данным.',
    modes: { cards: 'Карточки', chess: 'Шахматка' }, modeLabel: 'Режим каталога', language: 'Язык', filters: 'Фильтры', phase: 'Тип предложения', allTypes: 'Все предложения', apartmentType: 'Готовый дом · квартиры', rooms: 'Комнаты', allRooms: 'Все', areaFrom: 'Площадь от', areaTo: 'Площадь до', status: 'Статус', allPublished: 'Все опубликованные', available: 'В продаже', floor: 'Этаж', allFloors: 'Все этажи', sort: 'Сортировка', reset: 'Сбросить', result: 'предложений',
    sorts: { priceAsc: 'Сначала дешевле', priceDesc: 'Сначала дороже', areaAsc: 'Площадь по возрастанию', areaDesc: 'Площадь по убыванию', floorAsc: 'Сначала нижние этажи' }, snapshot: 'Актуальные данные', captured: 'Последнее обновление', sourceNote: 'Состав предложений, цены и статусы обновляются автоматически. Наличие и условия подтверждает отдел продаж.',
    active: 'В продаже', currentPrice: 'Цена каталога по актуальным данным', regularPrice: 'Цена до акции', perM2: 'за м²', apartment: '3-комн.', entrance: 'подъезд', plan: 'Увеличить планировку', choose: 'Выбрать квартиру', showMore: 'Показать ещё', noResults: 'По этим параметрам предложений нет.', resetFilters: 'Сбросить фильтры', previous: 'Прокрутить влево', next: 'Прокрутить вправо', offers: 'Опубликованные предложения', selected: 'Выбранная квартира', details: 'Детали квартиры', close: 'Закрыть', consult: 'Получить консультацию', sourceCaveat: 'Пустые позиции и недоступные статусы не выдумываются.',
  },
  uz: {
    back: 'Loyiha haqida', project: 'FLAGMAN · TAYYOR UY', title: 'Xususiy galereyadagi', accent: 'xonadonni tanlang.', lead: 'Rasmiy katalogdagi haqiqiy takliflar — joriy aniq rejalar, maydonlar, qavatlar va narxlar bilan.',
    modes: { cards: 'Kartalar', chess: 'Shaxmatka' }, modeLabel: 'Katalog ko‘rinishi', language: 'Til', filters: 'Filtrlar', phase: 'Taklif turi', allTypes: 'Barcha takliflar', apartmentType: 'Tayyor uy · xonadonlar', rooms: 'Xonalar', allRooms: 'Barchasi', areaFrom: 'Maydon, dan', areaTo: 'Maydon, gacha', status: 'Holat', allPublished: 'Barcha e’lonlar', available: 'Sotuvda', floor: 'Qavat', allFloors: 'Barcha qavatlar', sort: 'Saralash', reset: 'Tozalash', result: 'ta taklif',
    sorts: { priceAsc: 'Arzonidan boshlab', priceDesc: 'Qimmatidan boshlab', areaAsc: 'Maydon o‘sishi bo‘yicha', areaDesc: 'Maydon kamayishi bo‘yicha', floorAsc: 'Quyi qavatlar avval' }, snapshot: 'Ma’lumotlar yangilanadi', captured: 'So‘nggi yangilanish', sourceNote: 'Takliflar tarkibi, narxlar va holatlar avtomatik yangilanadi. Mavjudlik va shartlarni savdo bo‘limi tasdiqlaydi.',
    active: 'Sotuvda', currentPrice: 'Joriy katalog narxi', regularPrice: 'Aksiyagacha narx', perM2: 'm² uchun', apartment: '3-xonali', entrance: 'kirish', plan: 'Rejani kattalashtirish', choose: 'Xonadonni tanlash', showMore: 'Yana ko‘rsatish', noResults: 'Bu parametrlar bo‘yicha taklif yo‘q.', resetFilters: 'Filtrlarni tozalash', previous: 'Chapga surish', next: 'O‘ngga surish', offers: 'E’lon qilingan takliflar', selected: 'Tanlangan xonadon', details: 'Xonadon tafsilotlari', close: 'Yopish', consult: 'Maslahat olish', sourceCaveat: 'Bo‘sh joylar va mavjud bo‘lmagan holatlar to‘qib chiqarilmaydi.',
  },
  en: {
    back: 'About the project', project: 'FLAGMAN · COMPLETED', title: 'Choose an apartment', accent: 'in a private gallery.', lead: 'Real official-catalogue listings with exact plans, areas, floors and prices at the latest update.',
    modes: { cards: 'Cards', chess: 'Matrix' }, modeLabel: 'Catalogue view', language: 'Language', filters: 'Filters', phase: 'Listing type', allTypes: 'All listings', apartmentType: 'Completed · apartments', rooms: 'Rooms', allRooms: 'All', areaFrom: 'Area from', areaTo: 'Area to', status: 'Status', allPublished: 'All published', available: 'For sale', floor: 'Floor', allFloors: 'All floors', sort: 'Sort', reset: 'Reset', result: 'listings',
    sorts: { priceAsc: 'Lowest price first', priceDesc: 'Highest price first', areaAsc: 'Area ascending', areaDesc: 'Area descending', floorAsc: 'Lower floors first' }, snapshot: 'Automatically updated', captured: 'Last updated', sourceNote: 'Listings, prices and statuses update automatically. The sales team confirms availability and terms.',
    active: 'For sale', currentPrice: 'Catalogue price at latest update', regularPrice: 'Pre-offer price', perM2: 'per m²', apartment: '3-room', entrance: 'entrance', plan: 'Enlarge plan', choose: 'Choose apartment', showMore: 'Show more', noResults: 'No listings match these parameters.', resetFilters: 'Reset filters', previous: 'Scroll left', next: 'Scroll right', offers: 'Published listings', selected: 'Selected apartment', details: 'Apartment details', close: 'Close', consult: 'Request a consultation', sourceCaveat: 'Missing positions and unavailable statuses are not invented.',
  },
} as const;
...
function UnitDetail({ unit, language, onPlan, onLead }: { unit: FlagmanUnit; language: Language; onPlan: () => void; onLead: () => void }) {
  ...
  return <aside className="flagman-unit-detail" aria-label={t.details}>
    ...
    <div className="flagman-unit-detail__price"><small>{t.currentPrice}</small><strong>{money(unit.price, language)}</strong><span>{money(unit.pricePerM2, language)} {t.perM2}</span>{unit.regularPrice > unit.price ? <del>{money(unit.regularPrice, language)}</del> : null}</div>
...
export function FlagmanCatalogPage({ snapshot: embeddedSnapshot, initialLanguage = 'ru' }: { snapshot: FlagmanCatalogSnapshot; initialLanguage?: Language }) {
  const { data: snapshot } = useLiveCatalogSnapshot('flagman', embeddedSnapshot);
  ...
  useEffect(() => { if (leadUnit) rememberLiveCatalogUnit(leadUnit, 'flagman'); }, [leadUnit]);
  ...
  return <main className="flagman-catalog" lang={language}>
    ...
    <footer className="flagman-catalog-footer"><a className="flagman-catalog-wordmark" href={withLanguage('/flagman', language)}>FLAGMAN<small>TASHKENT</small></a><p>{t.sourceNote}</p><a href="tel:+998781137712">+998 78 113 77 12</a></footer>
    {planUnit ? <PlanLightbox unit={planUnit} language={language} onClose={() => setPlanUnit(undefined)} /> : null}
    <LeadModal open={Boolean(leadUnit)} language={language} context={leadUnit ? `flagman:catalog:unit:${leadUnit.number}:source:${leadUnit.id}` : 'flagman:catalog'} projectName="FLAGMAN" hideBrand tagline={leadUnit ? `№ ${leadUnit.number} · ${leadUnit.rooms} · ${leadUnit.area.toFixed(2)} м²` : t.accent} facts={leadUnit ? [snapshot.project.class, `${leadUnit.floor}/${leadUnit.maxFloor} ${t.floor}`, money(leadUnit.price, language)] : undefined} submitUrl={leadSubmitUrl()} projectSlug="flagman" {...catalogLeadIdentity(leadUnit)} privacyUrl={privacyUrl(language)} requireConsent onClose={() => setLeadUnit(undefined)} />
  </main>;
}
