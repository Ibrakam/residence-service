...
const copy = {
  ru: {
    ...
    progressIndex: 'BAYT 05 · СТРОЙКА · ИЮЛЬ 2026', progressTitle: 'Факт, привязанный к месяцу.', progressText: 'В июле 2026 года шли работы над монолитным каркасом, подготовкой перекрытия 7 этажа и параллельной кладкой стен. Фотографии ниже — реальная стройка очереди 2.2, а не утверждение о её готовности.', progressFact: 'Монолит · перекрытие 7 этажа · кладка стен',
    ...
    catalogIndex: 'BAYT 07 · КАТАЛОГ ОБНОВЛЯЕТСЯ', catalogTitle: 'Актуальные позиции и официальные статусы.', catalogText: 'В каталоге показаны предложения с официальными статусами, но статус не гарантирует юридическую доступность. Её можно уточнить из карточки квартиры.', catalogStats: [['Актуально', 'позиций'], ['Онлайн', 'свободно'], ['Официально', 'статусы'], ['Автоматически', 'цены'], ['Точно', 'планировки'], ['Подтверждаются', 'условия']] as const, catalogGroups: ['Опубликованные очереди', 'Актуальные предложения', 'Текущие площади', 'Комнаты и этажи в каталоге'], openCatalog: 'Открыть каталог', catalogNote: 'Цены, статусы и действующие условия обновляются автоматически. Финальные условия подтверждает отдел продаж.',
    ...
    formTagline: 'Вдохновлённый поэзией.', formFacts: ['Бизнес-класс', 'Актуальный каталог', 'финальная очередь 2.2'] as const,
  ...
  uz: {
    ...
    catalogIndex: 'BAYT 07 · KATALOG YANGILANADI', catalogTitle: 'Dolzarb pozitsiyalar va rasmiy holatlar.', catalogText: 'Katalog rasmiy holatlardagi takliflarni ko‘rsatadi, ammo holat huquqiy mavjudlikni kafolatlamaydi. Uni xonadon kartasidan aniqlashtirish mumkin.', catalogStats: [['Dolzarb', 'pozitsiya'], ['Onlayn', 'mavjud'], ['Rasmiy', 'holatlar'], ['Avtomatik', 'narxlar'], ['Aniq', 'rejalar'], ['Tasdiqlanadi', 'shartlar']] as const, catalogGroups: ['E’lon qilingan bosqichlar', 'Dolzarb takliflar', 'Joriy maydonlar', 'Katalogdagi xona va qavatlar'], openCatalog: 'Katalogni ochish', catalogNote: 'Narxlar, holatlar va amaldagi shartlar avtomatik yangilanadi. Yakuniy shartlarni savdo bo‘limi tasdiqlaydi.',
    ...
    formTagline: 'She’riyatdan ilhomlangan.', formFacts: ['Biznes-klass', 'Yangilanadigan katalog', 'yakuniy 2.2 bosqich'] as const,
  ...
  en: {
    ...
    catalogIndex: 'BAYT 07 · CATALOGUE UPDATES AUTOMATICALLY', catalogTitle: 'Current entries and official statuses.', catalogText: 'The catalogue shows listings with their official statuses; a technical status does not guarantee legal availability. Check it from the apartment card.', catalogStats: [['Current', 'entries'], ['Online', 'available'], ['Official', 'statuses'], ['Automatic', 'prices'], ['Exact', 'floor plans'], ['Confirmed', 'terms']] as const, catalogGroups: ['Published phases', 'Current listings', 'Current areas', 'Rooms and floors in the catalogue'], openCatalog: 'Open the catalogue', catalogNote: 'Prices, statuses and current terms update automatically. The sales team confirms final terms.',
    ...
    formTagline: 'Inspired by poetry.', formFacts: ['Business class', 'Current catalogue', 'final phase 2.2'] as const,
...
export function JomiyPage({ initialLanguage }: { initialLanguage: Language }) {
  ...
  const { data: catalogProject, dataSource } = useLiveCatalogProject('jomiy', { slug: 'jomiy', name: 'Jomiy', totalUnits: 0, availableUnits: 0 });
  ...
  const t = copy[language];
  const countLocale = language === 'ru' ? 'ru-RU' : language === 'uz' ? 'uz-UZ' : 'en-US';
  const catalogStats = t.catalogStats.map(([value, label], index) => [
    dataSource === 'embedded' || index > 1
      ? value
      : new Intl.NumberFormat(countLocale).format(index === 0 ? catalogProject.totalUnits : catalogProject.availableUnits),
    label,
  ] as const);
  const catalogTitle = dataSource === 'embedded' ? t.catalogTitle : language === 'ru'
    ? `${catalogProject.totalUnits} позиций · ${catalogProject.availableUnits} свободно.`
    : language === 'uz'
      ? `${catalogProject.totalUnits} ta pozitsiya · ${catalogProject.availableUnits} ta mavjud.`
      : `${catalogProject.totalUnits} entries · ${catalogProject.availableUnits} available.`;
