export type JomiyLanguage = 'ru' | 'uz' | 'en';

let bodyLockDepth = 0;
let previousOverflow = '';
let previousPaddingRight = '';

export function lockJomiyBody() {
  if (bodyLockDepth === 0) {
...
export function jomiyLocale(language: JomiyLanguage) {
  return language === 'ru' ? 'ru-RU' : language === 'uz' ? 'uz-UZ' : 'en-US';
}

export function jomiyLanguageTag(language: JomiyLanguage) {
