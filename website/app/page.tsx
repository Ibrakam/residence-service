...
function ApartmentModal({ unit, language, onClose, onPrint, onLead }: { unit: Apartment; language: Language; onClose: () => void; onPrint: () => void; onLead: () => void }) {
  ...
  const enlarge = language === 'ru' ? 'Увеличить планировку' : 'Rejani kattalashtirish';
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="apartment-modal" role="dialog" aria-modal="true" aria-labelledby="detail-title"><button className="modal-close" type="button" aria-label="Close" onClick={onClose}>×</button><div className="modal-heading"><div><p>{t.apartmentInfo}</p><h2 id="detail-title">{t.apartment} №{unit.number}</h2></div><span className={`status-chip status-chip--${unit.status}`}>{t.statuses[unit.status]}</span></div><div className="modal-unit-card"><button className="modal-plan modal-plan--button" type="button" onClick={onPrint} aria-label={`${enlarge} №${unit.number}`}><picture><source media="(max-width: 850px)" srcSet={assetPath('/avalon-apartment-plan-mobile.webp')} /><img src={assetPath('/avalon-apartment-plan.png')} alt={`${t.floorPlan} №${unit.number}`} loading="lazy" decoding="async" /></picture><span className="plan-zoom-hint">⤢ {enlarge}</span></button><div><span className="room-badge">{unit.rooms}к</span><h3>AVALON RESIDENCE · {unit.building}</h3><p>{unit.floor} {t.floor.toLowerCase()} · № {unit.number}</p><strong>{formatMoney(unit.price)}</strong><small>{unit.area} m² · {formatMoney(unit.pricePerM2)} / m²</small></div></div><div className="payment-heading"><h3 className="payment-title">{t.installment}</h3><span>{language === 'ru' ? 'Листайте условия' : 'Shartlarni suring'} <b>→</b></span></div><div className="payment-grid" tabIndex={0} aria-label={language === 'ru' ? 'Условия оплаты — листайте горизонтально' : 'To‘lov shartlari — gorizontal suring'}><article><span>₿</span><small>{t.total}</small><strong>{formatMoney(unit.price)}</strong></article><article><span>◫</span><small>{t.downPayment} · {downPaymentPercent}%</small><strong>{formatMoney(downPayment)}</strong></article><article><span>◇</span><small>{t.monthly} · {term} {t.months}</small><strong>{formatMoney(monthly)}</strong></article></div><div className="installment-calculator"><div className="calculator-controls"><label><span>{t.downPaymentUzs}</span><input type="number" min="0" max={price || undefined} step="1000000" value={downPaymentInput} onChange={(event) => setDownPaymentInput(event.target.value)} /></label><label><span>{t.installmentTerm}: <strong>{term} {t.months}</strong></span><input type="range" min="24" max="36" step="1" value={term} onChange={(event) => setTerm(Number(event.target.value))} /></label><div className="term-presets">{[24, 30, 36].map((months) => <button key={months} type="button" className={term === months ? 'is-active' : ''} onClick={() => setTerm(months)}>{months} {language === 'ru' ? 'мес.' : 'oy'}</button>)}</div></div><div className="calculator-result"><span>{t.saleAmount}</span><strong>{formatMoney(balance)}</strong><span>{t.installmentTerm}</span><strong>{term} {t.months}</strong><span>{t.monthly}</span><strong>{formatMoney(monthly)}</strong></div></div><div className="modal-actions"><button type="button" onClick={onPrint}>{t.printOffer}</button><button className="modal-actions__lead" type="button" onClick={onLead}>{t.contactTencorp} ↗</button></div><small className="calculation-disclaimer">{t.disclaimer}</small></section></div>;
}
