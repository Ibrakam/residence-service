...
import { regnumQueueCode, regnumQueueKey, regnumQueueLabel, regnumQueueOrder } from '../regnum-queues';
...
type MatrixGroup = { id: string; queueCode: string; queueOrder: number; section: string; units: RegnumUnit[]; rows: Array<{ id: string; floor: number; units: RegnumUnit[] }> };
...
function buildRegnumMatrixGroups(units: readonly RegnumUnit[]): MatrixGroup[] {
  const groups = new Map<string, Omit<MatrixGroup, 'rows'>>();
  units.forEach((unit) => {
    const queueKey = regnumQueueKey(unit) ?? 'unassigned';
    const section = String(unit.section);
    const id = `${queueKey}-s${section}`;
    const existing = groups.get(id);
    if (existing) existing.units.push(unit);
    else groups.set(id, {
      id,
      queueCode: regnumQueueCode(unit) ?? '—',
      queueOrder: regnumQueueOrder(unit),
      section,
      units: [unit],
    });
  });
  return [...groups.values()]
    .sort((left, right) => left.queueOrder - right.queueOrder || left.section.localeCompare(right.section, undefined, { numeric: true }))
    .map((group) => ({
      ...group,
      rows: [...new Set(group.units.map((unit) => unit.floor))]
        .sort((left, right) => left - right)
        .map((floor) => ({
          id: `${group.id}-f${floor}`,
          floor,
          units: group.units.filter((unit) => unit.floor === floor),
        })),
    }));
}
...
function PlanLightbox({ selection, language, covered, onClose, onLead }: { selection: Selection; language: Language; covered: boolean; onClose: () => void; onLead: (unit: RegnumUnit, opener: HTMLButtonElement) => void }) {
  ...
  return <div className="rpc-plan" role="dialog" aria-modal={covered ? undefined : true} aria-hidden={covered || undefined} inert={covered ? true : undefined} aria-labelledby="rpc-plan-title">
    ...
    <div ref={dialogRef} className="rpc-plan__dialog">
      ...
      <footer><dl><div><dt>{unit.rooms}</dt><dd>{t.roomsShort}</dd></div><div><dt>{unit.area.toLocaleString(regnumLocale(language))} м²</dt><dd>{t.area}</dd></div><div><dt>{unit.floor}</dt><dd>{t.floorShort}</dd></div><div><dt>{regnumQueueLabel(unit, language)} · {t.sectionShort} {unit.section}</dt><dd>{unit.completion}</dd></div></dl><div><strong>{priceOnRequest(language)}</strong><button type="button" data-lead-trigger onClick={(event) => onLead(unit, event.currentTarget)}>{t.planLead}<span>↗</span></button></div></footer>
    </div>
  </div>;
}
...
function UnitDetail({ selection, language, mobile, covered, onClose, onPlan, onLead }: { selection: Selection; language: Language; mobile: boolean; covered: boolean; onClose: () => void; onPlan: (selection: Selection) => void; onLead: (unit: RegnumUnit, opener: HTMLButtonElement) => void }) {
  ...
  const content = <aside ref={panelRef} className="rpc-detail" role={mobile ? 'dialog' : 'region'} aria-modal={mobile && !covered ? true : undefined} aria-hidden={!mobile && covered ? true : undefined} inert={!mobile && covered ? true : undefined} aria-labelledby="rpc-detail-title">
    ...
    <dl><div><dt>{t.area}</dt><dd>{unit.area.toLocaleString(regnumLocale(language))} м²</dd></div><div><dt>{t.rooms}</dt><dd>{unit.rooms}</dd></div><div><dt>{t.floorShort}</dt><dd>{unit.floor}</dd></div><div><dt>{t.queueShort} / {t.sectionShort}</dt><dd>{regnumQueueCode(unit) ?? '—'} / {unit.section}</dd></div><div><dt>{t.completionShort}</dt><dd>{unit.completion}</dd></div><div><dt>{t.status}</dt><dd>{t.available}</dd></div></dl>
...
function Matrix({ units, matchedIds, rankById, language, selection, onSelection }: { units: RegnumUnit[]; matchedIds: Set<string | number>; rankById: ReadonlyMap<string | number, number>; language: Language; selection: Selection | null; onSelection: (selection: Selection | null) => void }) {
  const t = copy[language]; const scrollRef = useRef<HTMLDivElement>(null); const [edges, setEdges] = useState({ left: true, right: false }); const groups = useMemo(() => buildRegnumMatrixGroups(units), [units]);
  ...
  return <div className="rpc-matrix-layout">
    <section className="rpc-matrix" aria-labelledby="rpc-matrix-title">
      ...
      <div ref={scrollRef} className="rpc-matrix__scroll" tabIndex={0} onKeyDown={keyScroll} aria-label={t.matrixHelp}>
        {groups.map((group) => {
          const groupMatchCount = group.units.filter((unit) => matchedIds.has(unit.id)).length;
          return <section key={group.id} className="rpc-matrix-panel" aria-labelledby={`rpc-group-${group.id}`}>
            <header><div><span>{group.queueCode}</span><h3 id={`rpc-group-${group.id}`}>{t.queueShort} {group.queueCode} · {t.sectionShort} {group.section}</h3></div><strong>{groupMatchCount} {t.matched}</strong></header>
            <div className="rpc-matrix-panel__rows">{group.rows.map((row) => {
              const orderedUnits = [...row.units].sort((left, right) => {
                const leftRank = rankById.get(left.id); const rightRank = rankById.get(right.id);
                if (leftRank !== undefined && rightRank !== undefined) return leftRank - rightRank;
                if (leftRank !== undefined) return -1;
                if (rightRank !== undefined) return 1;
                return left.sourceOrder - right.sourceOrder;
              });
              return <div className="rpc-matrix-row" key={row.id} data-row-id={row.id}><span>{t.row} {row.floor}</span><div>{orderedUnits.map((unit) => { const matches = matchedIds.has(unit.id); return <button key={unit.id} data-unit-id={unit.id} type="button" className={`${matches ? '' : 'is-filtered'} ${selection?.unit.id === unit.id ? 'is-selected' : ''}`} disabled={!matches} onClick={(event) => activate(unit, event.currentTarget)} aria-label={`${t.openUnit}: ${t.number}${unit.number}, ${unit.rooms} ${t.roomsShort}, ${unit.area} m²`}><strong>№{unit.number}</strong><small>{unit.rooms} {t.roomsShort} · {unit.area} м²</small><em>{priceOnRequest(language)}</em></button>; })}</div></div>;
            })}</div>
          </section>;
        })}
      </div>
...
export function RegnumCatalog({ snapshot: embeddedSnapshot, initialLanguage }: { snapshot: Snapshot; initialLanguage: Language }) {
  const { data: snapshot, project } = useLiveCatalogSnapshot('regnum-plaza', embeddedSnapshot);
  const units = snapshot.units as RegnumUnit[];
  const queueOptions = useMemo(() => liveCatalogQueueOptions(project), [project]);
  const queueCounts = useMemo(() => new Map(queueOptions.map((queue) => [queue.queueKey, units.filter((unit) => unit.queueKey === queue.queueKey).length])), [queueOptions, units]);
  const [language, setLanguage] = useLanguage(initialLanguage); const mobile = useMobile(); const t = copy[language]; const q = queueCopy[language];
  const router = useRouter(); const pathname = usePathname(); const searchParams = useSearchParams();
  const requestedQueue = searchParams.get('queue')?.trim() || null;
  const selectedQueue = queueOptions.some((queue) => queue.queueKey === requestedQueue && (queueCounts.get(queue.queueKey) ?? 0) > 0) ? requestedQueue! : 'all';
  const heroFacts = useMemo(() => {
    const rooms = units.map((unit) => unit.rooms).filter(Number.isFinite);
    const areas = units.map((unit) => unit.area).filter(Number.isFinite);
    ...
    const areaRange = areas.length
      ...
      : '—';
    return [[String(units.length), t.facts[0][1]], [roomRange, t.facts[1][1]], [areaRange, t.facts[2][1]], t.facts[3]];
  }, [language, t.facts, units]);
  ...
  const unique = useCallback((field: 'rooms' | 'floor' | 'queue' | 'section' | 'completion' | 'status') => [...new Set(units.map((unit) => String(unit[field])))].sort((left, right) => Number(left) - Number(right)), [units]);
  const filtered = useMemo(() => units.filter((unit) => {
    ...
    return (filters.rooms === 'all' || String(unit.rooms) === filters.rooms)
      ...
      && (queueOptions.length ? selectedQueue === 'all' || unit.queueKey === selectedQueue : filters.queue === 'all' || String(unit.queue) === filters.queue)
  ...
  }), [filters, queueOptions.length, selectedQueue, units]);
  ...
  const chooseQueue = (queueKey: string) => {
    const params = new URLSearchParams(searchParams.toString());
    params.set('lang', language);
    if (queueKey === 'all') params.delete('queue'); else params.set('queue', queueKey);
    router.replace(`${pathname}?${params.toString()}`, { scroll: false });
    resetResults();
  };
  const reset = () => { setFilters(defaultFilters); chooseQueue('all'); };
  ...
  useEffect(() => {
    if (!project || !queueOptions.length || !requestedQueue || selectedQueue !== 'all') return;
    const params = new URLSearchParams(searchParams.toString());
    params.set('lang', language); params.delete('queue');
    router.replace(`${pathname}?${params.toString()}`, { scroll: false });
  }, [language, pathname, project, queueOptions.length, requestedQueue, router, searchParams, selectedQueue]);
  ...
  return <div className="rpc-site" lang={language}>
    ...
    <main aria-hidden={mobileDetailOpen || undefined} inert={mobileDetailOpen ? true : undefined}>
      ...
      <section className="rpc-catalog" aria-labelledby="rpc-catalog-title">
        {queueOptions.length ? <section className="rpc-queues" aria-labelledby="rpc-queue-title">
          <header><span>{q.hint}</span><h2 id="rpc-queue-title">{q.title}</h2></header>
          <div role="group" aria-labelledby="rpc-queue-title">
            <button type="button" className={selectedQueue === 'all' ? 'is-active' : ''} aria-pressed={selectedQueue === 'all'} onClick={() => chooseQueue('all')}><strong>{q.all}</strong><small><b>{units.length}</b> {q.available}</small></button>
            {queueOptions.map((queue) => { const count = queueCounts.get(queue.queueKey) ?? 0; const disabled = count === 0; return <button type="button" key={queue.queueKey} className={selectedQueue === queue.queueKey ? 'is-active' : ''} aria-pressed={selectedQueue === queue.queueKey} disabled={disabled} title={disabled ? q.unavailable : undefined} onClick={() => chooseQueue(queue.queueKey)}><strong>{regnumQueueLabel(queue, language, queueOptions)}</strong><small><b>{count}</b> {disabled ? q.unavailable : q.available}</small></button>; })}
          </div>
        </section> : null}
        ...
        <form className="rpc-filters" onSubmit={(event) => event.preventDefault()}>
          ...
          <div className="rpc-filters__grid">
            ...
            {queueOptions.length ? null : <label><span>{t.queue}</span><select value={filters.queue} onChange={(event) => setFilter('queue', event.target.value)}><option value="all">{t.any}</option>{unique('queue').map((value) => <option key={value} value={value}>{regnumQueueLabel({ queue: value }, language)}</option>)}</select></label>}
        ...
        <div id="rpc-results" role="tabpanel" aria-labelledby={`rpc-tab-${mode}`} tabIndex={0}>
          {!sorted.length ? <section className="rpc-empty"><span>00</span><h2>{t.emptyTitle}</h2><p>{t.emptyText}</p><div><button type="button" onClick={reset}>{t.reset}</button><button type="button" data-lead-trigger onClick={() => openLead('catalog:empty')}>{t.emptyLead}<span>↗</span></button></div></section> : mode === 'cards' ? <>
            <div className="rpc-cards">{sorted.slice(0, visible).map((unit) => <article key={unit.id}>
              ...
              <dl><div><dt>{t.area}</dt><dd>{unit.area.toLocaleString(regnumLocale(language))} м²</dd></div><div><dt>{t.floorShort}</dt><dd>{unit.floor}</dd></div><div><dt>{t.queueShort} / {t.sectionShort}</dt><dd>{regnumQueueCode(unit) ?? '—'} / {unit.section}</dd></div><div><dt>{t.completionShort}</dt><dd>{unit.completion}</dd></div></dl>
            ...
            <Matrix units={units} matchedIds={matchedIds} rankById={rankById} language={language} selection={selection} onSelection={setSelection} />
    ...
    {lead ? <LeadModal open language={language} context={regnumLeadContext(lead.surface, language, lead.unit)} brandName="MURAD BUILDINGS" projectName="REGNUM PLAZA" tagline={t.formTagline} facts={lead.unit ? [`${regnumQueueLabel(lead.unit, language, queueOptions)} · ${t.sectionShort} ${lead.unit.section}`, `${lead.unit.rooms} ${t.roomsShort}`, `${lead.unit.area.toLocaleString(regnumLocale(language))} m²`] : t.formFacts} submitUrl={regnumLeadSubmitUrl()} projectSlug="regnum-plaza" {...catalogLeadIdentity(lead.unit)} privacyUrl={`${appBasePath}/privacy?project=regnum-plaza&lang=${language}&from=catalog`} requireConsent returnFocusTo={lead.opener} onClose={closeLead} /> : null}
  </div>;
}
