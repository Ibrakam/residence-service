...
const copy = {
  ru: {
    ...
    videoTitle: '24 секунды реального пространства', videoText: 'Официальное видео открытой первой очереди · без звука.', tour: 'Открыть официальный 360-тур', sourceArticle: 'Официальная статья', openGallery: 'Открыть галерею',
    genplanEyebrow: '03 · ГЕНПЛАН', genplanTitle: 'Один адрес. Одиннадцать блоков.', genplanText: 'CGI показывает полный проект, а цифры относятся к официальной карточке Regnum Plaza. Визуализация не является фотографией готового комплекса.', genplanFacts: [['11', 'блоков'], ['776', 'квартир'], ['3', 'очереди'], ['30 000 м²', 'территория']] as const, interactive: 'Официальный интерактивный генплан',
    ...
    contactEyebrow: '08 · КОНСУЛЬТАЦИЯ', contactTitle: 'Выберите свой проём в город.', contactText: 'Менеджер перепроверит текущую доступность и подготовит подборку. Заявка не является бронированием.', office: 'Офис продаж', project: 'Проект', privacy: 'Обработка персональных данных', sources: 'Официальные источники', top: 'Наверх',
  ...
  uz: {
    ...
    videoTitle: 'Haqiqiy makonning 24 soniyasi', videoText: 'Ochilgan birinchi bosqichning rasmiy videosi · ovozsiz.', tour: 'Rasmiy 360-turni ochish', sourceArticle: 'Rasmiy maqola', openGallery: 'Galereyani ochish',
    genplanEyebrow: '03 · BOSH REJA', genplanTitle: 'Bir manzil. O‘n bir blok.', genplanText: 'CGI butun loyihani ko‘rsatadi, raqamlar esa Regnum Plaza rasmiy kartasidan. Vizualizatsiya tayyor majmuaning fotosurati emas.', genplanFacts: [['11', 'blok'], ['776', 'xonadon'], ['3', 'bosqich'], ['30 000 m²', 'hudud']] as const, interactive: 'Rasmiy interaktiv bosh reja',
    ...
    contactEyebrow: '08 · MASLAHAT', contactTitle: 'Shaharga o‘z oynangizni tanlang.', contactText: 'Menejer joriy mavjudlikni qayta tekshiradi va tanlov tayyorlaydi. Ariza bron hisoblanmaydi.', office: 'Savdo ofisi', project: 'Loyiha', privacy: 'Shaxsiy ma’lumotlarni qayta ishlash', sources: 'Rasmiy manbalar', top: 'Yuqoriga',
  ...
  en: {
    ...
    videoTitle: '24 seconds of actual space', videoText: 'Official video of the opened first phase · silent.', tour: 'Open the official 360 tour', sourceArticle: 'Official article', openGallery: 'Open gallery',
    genplanEyebrow: '03 · MASTERPLAN', genplanTitle: 'One address. Eleven blocks.', genplanText: 'The CGI shows the full project, while the figures come from the official Regnum Plaza record. It is not a photograph of a completed development.', genplanFacts: [['11', 'blocks'], ['776', 'apartments'], ['3', 'phases'], ['30,000 m²', 'site']] as const, interactive: 'Official interactive masterplan',
    ...
    contactEyebrow: '08 · CONSULTATION', contactTitle: 'Choose your aperture onto the city.', contactText: 'A manager will re-check current availability and prepare a selection. A request is not a reservation.', office: 'Sales office', project: 'Project', privacy: 'Personal data processing', sources: 'Official sources', top: 'Back to top',
    ...
    <main id="rp-main" aria-hidden={menuOpen || undefined} inert={menuOpen ? true : undefined}>
      ...
      <section id="reality" className="rp-section rp-reality">
        ...
        <div className="rp-reality__grid">
          ...
          <article className="rp-reality__card" data-reveal><button type="button" onClick={(event) => openLightbox(archiveSlides, 0, event.currentTarget)}><img src={asset(archiveSlides[0].src)} width="1218" height="850" loading="lazy" alt={archiveSlides[0].caption[language]} /><span>{mediaLabels['archival-cgi-concept'][language]}</span></button><h3>{t.realityCards[3]}</h3><p>{archiveSlides[0].caption[language]}</p><a href="https://mbc.uz/news/murad-buildings-zapustila-dolgozdannyi-prestiznyi-i-prevosxodiashhii-vse-ozidaniia-ziloi-kompleks-biznes-klassa-regnum-plaza" target="_blank" rel="noreferrer">{t.sourceArticle} ↗</a></article>
        </div>
      </section>
      ...
      <section id="genplan" className="rp-section rp-genplan">
        ...
        <div className="rp-genplan__copy" data-reveal><span>{t.genplanEyebrow}</span><h2>{t.genplanTitle}</h2><p>{t.genplanText}</p><dl>{t.genplanFacts.map(([value, label]) => <div key={value}><dt>{value}</dt><dd>{label}</dd></div>)}</dl><a href="https://mbc.uz/genplan/regnum-plaza" target="_blank" rel="noreferrer">{t.interactive}<b>↗</b></a></div>
      </section>
      ...
      <section className="rp-contact">
        ...
        <footer><img src={asset('/regnum-plaza/logo.svg')} width="522" height="95" alt="Regnum Plaza" /><p>{t.disclaimer}</p><nav><a href={`${appBasePath}/privacy?project=regnum-plaza&lang=${language}&from=landing`}>{t.privacy}</a><a href="https://mbc.uz/project/regnum-plaza" target="_blank" rel="noreferrer">{t.sources}</a><a href="#top">{t.top}</a></nav></footer>
      </section>
    </main>
