...
type Mode = 'cards' | 'matrix' | 'matrix-plus';
...
const copy = {
  ru: {
    ...
    modes: { cards: 'Карточки', matrix: 'Шахматка', 'matrix-plus': 'Шахматка+' }, filters: 'Фильтры', queue: 'Очередь', allQueues: 'Все очереди', class: 'Класс', allClasses: 'Business и Comfort', business: 'Business', comfort: 'Comfort', rooms: 'Комнаты', area: 'Площадь, м²', areaFrom: 'От', areaTo: 'До', status: 'Статус', allActive: 'В продаже', priceRequestOnly: 'Цена по запросу', reset: 'Сбросить', result: 'предложений',
...
function MatrixGroup({ units, language, mode, selectedId, onSelect, onPlan, onLead, index }: { units: Unit[]; language: Language; mode: Mode; selectedId?: string; onSelect: (unit: Unit) => void; onPlan: (unit: Unit) => void; onLead: (unit: Unit) => void; index: number }) {
  const t = copy[language];
  ...
  return <section className="sado-matrix-group">
    ...
    <div className={mode === 'matrix-plus' ? 'sado-matrix-plus-layout' : ''}>
      ...
      {mode === 'matrix-plus' && selected ? <UnitDetail unit={selected} language={language} onPlan={() => onPlan(selected)} onLead={() => onLead(selected)} /> : null}
    </div>
    {mode === 'matrix' && selected ? <div className="sado-matrix-selection"><div><small>{t.selected}</small><strong>№ {selected.number} · {selected.rooms} · {selected.area.toFixed(2)} м²</strong><span>{selected.floor}/{selected.maxFloor} {t.floor} · {selected.entrance} {t.entrance}</span></div><button type="button" onClick={() => onPlan(selected)}>{t.openPlan}</button><button className="is-primary" type="button" onClick={() => onLead(selected)}>{t.choose}</button></div> : null}
  </section>;
}
...
export function SadoCatalogPage({ initialUnits, snapshotGeneratedAt, sourceCount }: { initialUnits: Unit[]; snapshotGeneratedAt: string; sourceCount: number }) {
  ...
  return <main className="sado-catalog">
    ...
    <section className="sado-catalog-toolbar" aria-label={t.filters}>
      <div className="sado-mode-switch" role="radiogroup">{(['cards', 'matrix', 'matrix-plus'] as Mode[]).map((item) => <button key={item} data-testid={`mode-${item}`} type="button" role="radio" aria-checked={mode === item} className={mode === item ? 'is-active' : ''} onClick={() => setMode(item)}>{t.modes[item]}</button>)}</div>
    ...
    <section className="sado-catalog-results">
      ...
      {mode !== 'cards' && filtered.length ? <div className="sado-matrices">{groups.map((units, index) => <MatrixGroup key={units[0].block} units={units} language={language} mode={mode} selectedId={selectedId} onSelect={(unit) => setSelectedId(unit.id)} onPlan={setPlanUnit} onLead={setLeadUnit} index={index} />)}</div> : null}
    </section>
