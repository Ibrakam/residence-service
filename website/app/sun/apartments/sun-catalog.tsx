...
export function SunCatalog({ initialLanguage }: { initialLanguage: Language }) {
  ...
  return <div className="sunc-site" lang={language}>
    ...
    <header id="top" className="sunc-header" aria-hidden={rootCovered || undefined} inert={rootCovered ? true : undefined}><a href={sunPath('/sun', language)}><img src={sunAsset('/sun/logo.svg')} width="380" height="64" alt="SUN" /></a><nav aria-label={t.nav}><a href={sunPath('/sun', language)}>← {t.back}</a><a href="tel:+998781137712">+998 78 113 77 12</a></nav><div aria-label={t.language}>{sunLanguages.map((item) => <button type="button" key={item} className={language === item ? 'is-active' : ''} aria-pressed={language === item} onClick={() => setLanguage(item)}>{item.toUpperCase()}</button>)}</div><button className="sunc-header__lead" type="button" data-lead-trigger onClick={(event) => openLead('catalog:header', null, event.currentTarget)}>{t.consult}<span>↗</span></button><button ref={menuButtonRef} className="sunc-menu-button" type="button" aria-expanded={menuOpen} aria-controls="sunc-menu" onClick={() => setMenuOpen(true)}><span>{t.menu}</span><i /><i /></button></header>
    ...
    <main aria-hidden={rootCovered || undefined} inert={rootCovered ? true : undefined}>
      ...
      <section className="sunc-footer"><div><span>SUN · HUMAN2HUMAN</span><h2>{t.footerTitle}</h2><p>{t.footerText}</p><button type="button" data-lead-trigger onClick={(event) => openLead('catalog:footer', null, event.currentTarget)}>{t.consult}<b>↗</b></button></div><a href="tel:+998781137712">+998 78 113 77 12 <span>↗</span></a><footer><img src={sunAsset('/sun/logo.svg')} width="380" height="64" alt="SUN" /><nav><a href={`${sunAsset('/privacy')}?project=sun&lang=${language}&from=catalog`}>{t.privacy}</a><a href="#top">{t.top}</a></nav></footer></section>
    </main>
