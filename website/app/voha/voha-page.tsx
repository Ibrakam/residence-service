...
const copy = {
  ru: {
    ...
    serviceTop: '07 · Управление', serviceTitle: 'Сервис как спокойная ежедневная работа.', serviceText: 'Управляющая компания отвечает за чистоту, безопасность, инженерные системы и уход за ландшафтом.', serviceRate: 'Актуальные условия первоначального платежа для квартир, офисов и паркинга уточняет отдел продаж.',
    ...
    mediaTop: '09 · Смотреть проект', mediaTitle: 'Панорама и камера.', panorama: 'Открыть панораму 360', panoramaNote: 'Внешний сервис Uzbekistan360', camera: 'Открыть онлайн-камеру', cameraNote: 'Внешний поток RTSP.me; доступность зависит от сервиса', booklet: 'Скачать официальный буклет', bookletNote: 'PDF · 88 страниц · 24,8 МБ',
    ...
    catalogTop: '11 · Актуальные данные', catalogTitle: 'Актуальные предложения.', catalogText: 'Каталог автоматически обновляет планировки, очереди, площади, этажи, цены и статусы.', catalogNote: 'Цены, акции и статусы могут измениться у официального продавца.',
    ...
    formTitle: 'Получить консультацию', formName: 'Ваше имя', formPhone: 'Номер телефона', formSubmit: 'Отправить запрос', formNameError: 'Введите имя — не менее 2 символов.', formPhoneError: 'Введите 9 цифр номера после +998.', formSuccess: 'Спасибо. Запрос принят.', formContext: 'Контекст запроса',
  ...
  uz: {
    ...
    serviceTop: '07 · Boshqaruv', serviceTitle: 'Servis — kundalik sokin mehnat.', serviceText: 'Boshqaruv kompaniyasi tozalik, xavfsizlik, muhandislik tizimlari va landshaft parvarishiga javob beradi.', serviceRate: 'Xonadon, ofis va parking uchun amaldagi boshlang‘ich to‘lov shartlarini savdo bo‘limi aniqlashtiradi.',
    ...
    mediaTop: '09 · Loyihani ko‘ring', mediaTitle: 'Panorama va kamera.', panorama: '360 panoramani ochish', panoramaNote: 'Uzbekistan360 tashqi servisi', camera: 'Onlayn kamerani ochish', cameraNote: 'RTSP.me tashqi oqimi; mavjudlik servisga bog‘liq', booklet: 'Rasmiy bukletni yuklab olish', bookletNote: 'PDF · 88 sahifa · 24,8 MB',
    ...
    catalogTop: '11 · Yangilanadigan katalog', catalogTitle: 'Dolzarb takliflar.', catalogText: 'Rejalar, bosqichlar, maydonlar, qavatlar, narxlar va holatlar avtomatik yangilanadi.', catalogNote: 'Narxlar, aksiyalar va holatlar rasmiy sotuvchida o‘zgarishi mumkin.',
    ...
    formTitle: 'Maslahat olish', formName: 'Ismingiz', formPhone: 'Telefon raqami', formSubmit: 'So‘rov yuborish', formNameError: 'Ismni kiriting — kamida 2 ta belgi.', formPhoneError: '+998 dan keyin 9 ta raqam kiriting.', formSuccess: 'Rahmat. So‘rovingiz qabul qilindi.', formContext: 'So‘rov konteksti', photo: 'Haqiqiy surat', concept: 'Rasmiy konsepsiya', report: 'Arxiv foto hisoboti · 2026-yil iyul', previous: 'Oldingi surat', next: 'Keyingi surat',
  },
  en: {
    ...
    serviceTop: '07 · Management', serviceTitle: 'Service as calm, daily work.', serviceText: 'The management company covers cleanliness, safety, engineering systems and landscape care.', serviceRate: 'Ask the sales team to confirm current initial-payment terms for apartments, offices and parking.',
    ...
    mediaTop: '09 · Explore the project', mediaTitle: 'Panorama and camera.', panorama: 'Open 360 panorama', panoramaNote: 'External Uzbekistan360 service', camera: 'Open live camera', cameraNote: 'External RTSP.me stream; availability depends on the service', booklet: 'Download official brochure', bookletNote: 'PDF · 88 pages · 24.8 MB',
    ...
    catalogTop: '11 · Live catalogue', catalogTitle: 'Current listings.', catalogText: 'Plans, phases, areas, floors, prices and statuses update automatically.', catalogNote: 'Prices, promotions and statuses may change with the official seller.',
    ...
    formTitle: 'Request a consultation', formName: 'Your name', formPhone: 'Phone number', formSubmit: 'Send request', formNameError: 'Enter at least 2 characters.', formPhoneError: 'Enter 9 digits after +998.', formSuccess: 'Thank you. Your request has been received.', formContext: 'Request context', photo: 'Actual photography', concept: 'Official concept', report: 'Archived report · July 2026', previous: 'Previous photograph', next: 'Next photograph',
  },
} as const;
...
export function VohaPage({ initialLanguage = 'ru' }: { initialLanguage?: Language }) {
  ...
  const { data: catalogProject, dataSource } = useLiveCatalogProject('voha', { slug: 'voha', name: 'Voha', totalUnits: 0, availableUnits: 0 });
  const catalogCount = new Intl.NumberFormat(language === 'ru' ? 'ru-RU' : language === 'uz' ? 'uz-UZ' : 'en-US').format(catalogProject.availableUnits);
  const catalogTitle = dataSource === 'embedded' ? t.catalogTitle : language === 'ru'
    ? `${catalogCount} актуальных предложений.`
    : language === 'uz'
      ? `${catalogCount} ta dolzarb taklif.`
      : `${catalogCount} current listings.`;
  ...
  return <main className="voha-site" lang={language}>
    ...
    <header className="voha-header"><a className="voha-wordmark" href={withLanguage('/voha', language)} aria-label="Voha">VOHA<small>by NRG-BI</small></a><nav>{Object.entries(t.nav).map(([key, value]) => <a key={key} href={`#${key}`}>{value}</a>)}</nav><a className="voha-header__phone" href="tel:+998781137712">+998 78 113 77 12</a><button className="voha-menu-button" type="button" onClick={() => setMenuOpen(true)} aria-expanded={menuOpen}>{t.menu}<i /></button></header>
    ...
    <section className="voha-location" id="location" data-voha-reveal><div className="voha-location__copy"><p className="voha-overline">{t.locationTop}</p><h2>{t.locationTitle}</h2><p>{t.locationText}</p><a className="voha-button is-solid" href={mapLink} target="_blank" rel="noreferrer">{t.map}<span>↗</span></a></div><div className="voha-location__map"><span>VOHA</span><i /><small>Ташкент<br />Мирзо-Улугбекский район</small><a href={mapLink} target="_blank" rel="noreferrer">2GIS ↗</a></div><aside><small>{t.offices}</small><strong>ул. Нукус, 91/1</strong><strong>ул. Айбека, 38Б</strong><a href="tel:+998781137712">+998 78 113 77 12</a></aside></section>
    <section className="voha-media" data-voha-reveal><header><p className="voha-overline">{t.mediaTop}</p><h2>{t.mediaTitle}</h2></header><div><a href={panorama} target="_blank" rel="noreferrer"><span>360°</span><strong>{t.panorama}</strong><small>{t.panoramaNote}</small><b>↗</b></a><a href={camera} target="_blank" rel="noreferrer"><span>LIVE</span><strong>{t.camera}</strong><small>{t.cameraNote}</small><b>↗</b></a></div></section>
    ...
    <section className="voha-catalog-cta" data-voha-reveal><p className="voha-overline">{t.catalogTop}</p><div><h2>{catalogTitle}</h2><p>{t.catalogText}<small>{t.catalogNote}</small></p></div><a className="voha-button is-light" href={withLanguage('/voha/apartments', language)}>{t.choose}<span>↗</span></a></section>
    ...
    <footer className="voha-footer"><a className="voha-wordmark" href={withLanguage('/voha', language)}>VOHA<small>by NRG-BI</small></a><div><a href="tel:+998781137712">{t.phone} · +998 78 113 77 12</a><a href={`${appBasePath}/privacy?project=voha&lang=${language}`}>{t.privacy}</a></div><p>© 2026 · Информация не является публичной офертой.</p></footer>
