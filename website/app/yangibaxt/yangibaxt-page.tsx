...
const copy = {
  ru: {
    ...
    catalogIndex: '07 · КАТАЛОГ · 30.08.2026', catalogTitle: '265 записей — и только 63 со статусом «Свободно».', catalogText: 'В каталоге собраны только квартиры: 265 записей вместо общего счётчика 320, куда также входят паркинги, офисы и цоколи. Статусы резерва, брони и расторжения не выдаются за свободные предложения.', catalogStats: [['265', 'квартир на дату каталога'], ['63', 'Свободно + isSale'], ['5', 'групп каталога'], ['1–16', 'этажи предложений']] as const, catalogGroups: ['1 · 1 квартира', '2 · 1 квартира', '3-1 · 14 квартир', '3-2 · 141 квартира', 'Munavvar 1 · 108 квартир'] as const, openCatalog: 'Открыть полный каталог', catalogNote: 'Цены и кампании зафиксированы на 30.08.2026. У части квартир использована цена кампании, у 11 — исходная цена без кампании. Текущие условия подтверждает отдел продаж.',
  ...
  uz: {
    ...
    catalogIndex: '07 · KATALOG · 30.08.2026', catalogTitle: '265 ta yozuv — faqat 63 tasi “Bo‘sh” holatida.', catalogText: 'Katalogda faqat xonadonlar bor: parking, ofis va sokollarni ham o‘z ichiga olgan umumiy 320 hisobidan farqli ravishda 265 ta yozuv. Rezerv, bron va bekor qilish holatlari bo‘sh taklif sifatida ko‘rsatilmaydi.', catalogStats: [['265', 'katalog sanasidagi xonadon'], ['63', 'Bo‘sh + isSale'], ['5', 'katalog guruhi'], ['1–16', 'taklif qavatlari']] as const, catalogGroups: ['1 · 1 xonadon', '2 · 1 xonadon', '3-1 · 14 xonadon', '3-2 · 141 xonadon', 'Munavvar 1 · 108 xonadon'] as const, openCatalog: 'To‘liq katalogni ochish', catalogNote: 'Narxlar va kampaniyalar 30.08.2026 holatiga qayd etilgan. Ayrim xonadonlarda kampaniya narxi, 11 tasida kampaniyasiz asl narx ishlatilgan. Amaldagi shartlarni savdo bo‘limi tasdiqlaydi.',
  ...
  en: {
    ...
    catalogIndex: '07 · CATALOGUE · 30 AUG 2026', catalogTitle: '265 records — only 63 marked “Available”.', catalogText: 'The catalogue contains apartments only: 265 records rather than the overall count of 320, which also includes parking, offices and basement units. Reservation, booking and termination stages are not presented as available homes.', catalogStats: [['265', 'apartments on catalogue date'], ['63', 'Available + isSale'], ['5', 'catalogue groups'], ['1–16', 'listing floors']] as const, catalogGroups: ['1 · 1 apartment', '2 · 1 apartment', '3-1 · 14 apartments', '3-2 · 141 apartments', 'Munavvar 1 · 108 apartments'] as const, openCatalog: 'Open the full catalogue', catalogNote: 'Prices and campaigns were captured on 30 Aug 2026. Some apartments use a campaign price; 11 use the raw price because no campaign was present. The sales team confirms current terms.',
      ...
      <main id="main" aria-hidden={mobileMenuOpen || undefined} inert={mobileMenuOpen ? true : undefined}>
        ...
        <section className="yb-section yb-catalog-preview" id="catalog" data-reveal>
          ...
          <div className="yb-catalog-preview__panel">
            <dl>{t.catalogStats.map(([value, label]) => <div key={label}><dt>{value}</dt><dd>{label}</dd></div>)}</dl>
