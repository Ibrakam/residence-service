...
if (!/useLiveCatalogUnits\('c1'/.test(landing) || !/useLiveCatalogSnapshot\('c1'/.test(catalogue)) fail('C1 landing and catalogue must use live data with their embedded payloads as fallback.');
if (!/['"]@type['"]\s*:\s*['"]CollectionPage['"]/.test(cataloguePage) || !/['"]@type['"]\s*:\s*['"]ApartmentComplex['"]/.test(cataloguePage) || !/['"]@type['"]\s*:\s*['"]BreadcrumbList['"]/.test(cataloguePage)) fail('C1 catalogue JSON-LD must describe only the stable collection page, project and breadcrumbs.');
if (/['"]@type['"]\s*:\s*['"]ItemList['"]|itemListElement\s*:\s*catalog\.units\.map|numberOfItems\s*:\s*catalog\.units\.length|dateModified\s*:\s*catalog\.capturedAt/.test(cataloguePage)) fail('C1 catalogue JSON-LD must not publish embedded fallback units as live inventory.');
if (!/catalogLeadIdentity/.test(landing) || !/catalogLeadIdentity/.test(catalogue) || !/rememberLiveCatalogUnit/.test(landing) || !/rememberLiveCatalogUnit/.test(catalogue)) fail('C1 unit leads must use and remember the canonical live source key.');
if (/\bunitId\b|\bcrmId\b/.test(`${landing}\n${catalogue}\n${landingPage}\n${cataloguePage}`)) fail('C1 client code must not serialize legacy unitId or CRM identifiers.');
...
if (!/root\.lang = language/.test(language) || !/root\.lang === language/.test(language) || !/'\/c1\/:path\*'/.test(proxy)) fail('Document language proxy must cover all C1 document routes.');
...
if (!/availabilityStatus: 'ДОСТУПНА'/.test(catalogue) || !/availabilityStatus: 'MAVJUD'/.test(catalogue) || !/liveState: 'Обновляется автоматически'/.test(catalogue) || !/fallbackState: 'So‘nggi mavjud ma’lumotlar'/.test(catalogue)) fail('RU/UZ live catalogue state labels are not fully localized.');
...
if (/publicClientPayload/.test(`${landingPage}\n${cataloguePage}`) || /crmId|sourcePlanSha256|localPlanSha256|sourceCreatedAt|sourceUpdatedAt/.test(`${landingPage}\n${cataloguePage}`)) fail('Server pages expose CRM or capture-only provenance through the client payload.');
if (!/project\?\.availableUnits\s*\?\?\s*initialAvailableCount/.test(landing) || /свежем официальном (?:snapshot|снимке)|fresh official snapshot|Yangi rasmiy snapshotda/iu.test(`${landing}\n${catalogue}`)) fail('C1 availability copy must be dynamic and free of snapshot terminology.');
