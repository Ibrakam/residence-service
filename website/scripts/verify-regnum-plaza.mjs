...
async function verifyApplicationContract(catalog) {
  ...
  assertIncludes(source.catalogPage, ['alternates', 'languages', 'openGraph', 'twitter', 'ItemList', 'Apartment', 'dateModified: catalog.capturedAt', 'current.queueName', 'regnumQueueLabel(unit, language)', 'current.statusName', 'current.available'], 'catalog metadata/JSON-LD');
  ...
  assertIncludes(source.landingUi, ["const languages: Language[] = ['ru', 'uz', 'en']", 'real-first-phase', 'documentary-opening', 'cgi-full-project', 'archival-cgi-concept', 'const latest = useRef({ state, onChange })', 'returnFocusTo={lead.opener}', '!menuRef.current.contains(document.activeElement)', 'role="dialog"', 'aria-modal={menuOpen && !lead ? true : undefined}', 'inert={!menuOpen || Boolean(lead)', 'inert={menuOpen ? true : undefined}', 'width={unit.planWidth!}', 'height={unit.planHeight!}', 'liveCatalogQueueOptions(project)', 'regnumQueueLabel(unit, language, queueOptions)'], 'landing UI/layers');
  ...
  assert(source.catalogCss.includes('.rpc-card__plan img { position:absolute; inset:0; display:block; width:100%; height:100%; object-fit:contain;'), 'Card plan previews must fit within the fixed stage without intrinsic-ratio cropping');
  assert(source.sharedCss.includes(':is(.rp-site,.rpc-site)'), 'Shared Regnum lead styling is not scoped to both routes');
  assertIncludes(source.catalogUi, ["type Mode = 'cards' | 'chess'", "const modes: Mode[] = ['cards', 'chess']", "type Sort = 'source' | 'priceAsc' | 'priceDesc' | 'areaAsc' | 'areaDesc' | 'floorAsc' | 'floorDesc' | 'roomsAsc' | 'roomsDesc' | 'ppmAsc' | 'ppmDesc'", 'priceRank', 'ppmRank', 'rankById', 'data-row-id', 'data-unit-id', 'covered={Boolean(plan || lead)}', 'aria-hidden={covered || undefined}', 'inert={covered ? true : undefined}', 'returnFocusTo={lead.opener}', '!panel.contains(document.activeElement)', "const mobileDetailOpen = mode === 'chess'", 'inert={mobileDetailOpen ? true : undefined}', 'width={unit.planWidth!}', 'height={unit.planHeight!}', 'setVisible(12)', 'ArrowLeft', 'ArrowRight', 'Home', 'End', 'Escape', 'liveCatalogQueueOptions(project)', 'unit.queueKey === selectedQueue', 'function buildRegnumMatrixGroups'], 'catalog sorting/layer UI');
  assert(!source.catalogUi.includes('snapshot.matrix') && !source.catalogUi.includes('Q{unit.queue}') && !source.catalogUi.includes('<dd>{unit.queue}'), 'Regnum queue UI must use explicit display metadata and rebuild the matrix from current live units');
  ...
  assertIncludes(source.lead, ["@/data/regnum-plaza-client.json", "process.env.NODE_ENV === 'production'", '/v1/leads', '/api/regnum-plaza-lead', "['unitId', String(unit.id)]", 'RegnumQueueUnit', "rememberLiveCatalogUnit(unit, 'regnum-plaza')"], 'sanitized lead client');
  ...
  assertIncludes(source.leadModal, ['if (open && projectSlug && unitKey)', 'rememberLastViewedApartment({ unitKey }, projectSlug)', 'deliveryError', 'returnFocusTo', 'privacyUrl?', 'privacyUrl ? <> <a href={privacyUrl}'], 'shared lead migration/error/focus/privacy');
